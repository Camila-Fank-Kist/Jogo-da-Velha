...
def verificar_digito(caractere, qual):
  while True:
      digito = input(f'{caractere}, informe a {qual}: ').strip() # strip() -> remove espaços em branco
      if digito.isdigit(): # isdigit() -> verifica se uma string contém apenas dígitos numéricos
        digito = int(digito)
        if 0 <= digito <= 2:
          break
        else:
...
qtd_jogadas = 0
caractere_da_vez = ['X','O','X','O','X','O','X','O','X']

for i in caractere_da_vez:
  inserir(i)
  qtd_jogadas += 1
  imprimir(matriz)
